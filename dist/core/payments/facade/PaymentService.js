...
export class PaymentService {
     ...
     */
    static async charge(provider, input) {
        ...
        const estrategiaDecorada = new TelemetryDecorator(new RetryDecorator(estrategiaBase), provider);
        // Ejecutar comando a través del Command Bus
        return commandBus.dispatch(new CapturePaymentCommand(estrategiaDecorada, input, provider));
    }
    /**
     ...
     */
    static async refund(provider, input) {
        const estrategiaBase = PaymentFactory.create(provider);
        const estrategiaDecorada = new TelemetryDecorator(new RetryDecorator(estrategiaBase), provider);
        return commandBus.dispatch(new RefundPaymentCommand(estrategiaDecorada, input, provider));
    }
}
