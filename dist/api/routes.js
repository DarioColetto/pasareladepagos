 ...
 */
routes.post('/payments/charge', validateRequest(PaymentChargeSchema), async (request, response) => {
    try {
 ...
 */
routes.post('/payments/refund', validateRequest(PaymentRefundSchema), async (request, response) => {
    try {
...
function obtenerCodigoError(error) {
    ...
    if (error.codigo === 'FONDOS_INSUFICIENTES')
        return 402;
    if (error.codigo === 'ERROR_AUTENTICACION')
        return 401;
