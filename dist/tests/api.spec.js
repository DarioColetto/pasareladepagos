...
describe('API /payments', () => {
    ...
    it('POST /payments/charge → 200 y responde lo que devuelve PaymentService.charge', async () => {
        ...
        expect(PaymentService.charge).toHaveBeenCalledWith('stripe', {
            amount: 1500,
            currency: 'ARS',
            token: 'tok_123',
            metadata: undefined,
    ...
    it('POST /payments/charge → 500 cuando PaymentService.charge lanza un error', async () => {
        PaymentService.charge.mockRejectedValue(new Error('Service error'));
        const res = await request(app)
            .post('/payments/charge')
            .send({
            provider: 'stripe',
    ...
    it('POST /payments/refund → 200 y responde lo que devuelve PaymentService.refund', async () => {
        PaymentService.refund.mockResolvedValue({
            id: 're_123',
            status: 'refunded',
        });
        const res = await request(app)
        ...
        expect(PaymentService.refund).toHaveBeenCalledWith('stripe', {
            paymentId: 'ch_123',
            amount: 500,
        });
        expect(res.body).toEqual({
