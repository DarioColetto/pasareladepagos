...
export class FakeEventBus extends EventBus {
  public emittedEvents: Array<{ name: EventName; payload: any }> = [];
   ...
   */
  async emit<K extends EventName>(name: K, payload: EventPayloads[K]): Promise<void> {
   ...
   */
  getEventsByName<K extends EventName>(name: K): Array<EventPayloads[K]> {
   ...
   */
  wasEventEmittedWith<K extends EventName>(
    name: K, 
    expectedPayload: Partial<EventPayloads[K]>
  ): boolean {
    return this.emittedEvents.some(event => 
      event.name === name &&
      Object.keys(expectedPayload).every(key => 
        event.payload[key] === expectedPayload[key as keyof EventPayloads[K]]
      )
    );
  }
